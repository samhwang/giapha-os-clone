...
test.describe("Secondary Pages", () => {
  ...
  test("should display events page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /sự kiện/i })).toBeVisible({ timeout: 15000 });
  });

  test("should display kinship finder page", async ({ page }) => {
    await page.goto("/dashboard/kinship");
    await expect(page.getByRole("heading", { name: /tra cứu/i })).toBeVisible({ timeout: 15000 });
    await expect(page.getByText(/chọn thành viên/i).first()).toBeVisible();
  });

  test("should display lineage management page", async ({ page }) => {
    await page.goto("/dashboard/lineage");
    await expect(page.getByRole("heading", { name: /thế hệ/i })).toBeVisible({ timeout: 15000 });
