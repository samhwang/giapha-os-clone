...
async function exportToZipBuffer(data: {
  persons: Person[];
...
describe("CSV export/import via zip", () => {
  ...
  it("throws on invalid zip (missing files)", async () => {
    ...
    await expect(parseZipBuffer(buf)).rejects.toThrow(
      "Invalid ZIP: missing persons.csv or relationships.csv",
    );
  });
});
...
describe("exportToCsvZip / parseCsvZip (actual functions)", () => {
  ...
  it("parseCsvZip throws when required files are missing", async () => {
    ...
    zip.file("wrong.csv", "data");
    const buf = await zip.generateAsync({ type: "arraybuffer" });
    await expect(parseCsvZip(buf as unknown as Blob)).rejects.toThrow(
      "Invalid ZIP: missing persons.csv or relationships.csv",
    );
  });
});
