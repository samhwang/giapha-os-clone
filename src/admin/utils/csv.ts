...
export async function parseCsvZip(zipBlob: Blob): Promise<{
  ...
  if (privateFile) {
    const parsed = Papa.parse<Partial<PersonDetailsPrivateExport>>(
      stripBom(await privateFile.async("text")),
      {
        header: true,
        skipEmptyLines: true,
        dynamicTyping: true,
  ...
  if (eventsFile) {
    const parsed = Papa.parse<Partial<CustomEventExport>>(
      stripBom(await eventsFile.async("text")),
      {
        header: true,
        skipEmptyLines: true,
        dynamicTyping: true,
