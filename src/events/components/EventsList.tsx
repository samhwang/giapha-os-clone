...
function daysUntilLabel(
  ...
  if (days < -DAYS_MONTHS_AGO_THRESHOLD)
    return t("common.monthsAgo", { months: Math.ceil(Math.abs(days) / DAYS_PER_MONTH_APPROX) });
  if (days < -1) return t("common.daysAgo", { days: Math.abs(days) });

  if (days <= DAYS_PER_MONTH_APPROX) return t("common.daysFromNow", { days });
  if (days <= DAYS_MONTHS_AGO_THRESHOLD)
    return t("common.weeksFromNow", { weeks: Math.ceil(days / DAYS_PER_WEEK) });
  return t("common.monthsFromNow", { months: Math.ceil(days / DAYS_PER_MONTH_APPROX) });
}

function computeYearsInfo(
  event: FamilyEvent,
  t: (key: string, opts?: Record<string, unknown>) => string,
): string | null {
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate font-semibold text-stone-800 transition-colors group-hover:text-amber-700">
            {event.personName}
          </p>
...
export default function EventsList({
  ...
  const allEvents = useMemo(
    () => computeEvents({ persons, customEvents, lunarSuffix: t("common.lunarSuffix") }),
    [persons, customEvents, t],
  );
  ...
  return (
    <div className="space-y-5">
        ...
        {(todayCount > 0 || soonCount > 0) && (
          <div className="mt-2 flex items-center gap-2 border-t border-amber-200/40 pt-2">
            ...
            <p className="text-sm font-medium text-amber-800">
              {todayCount > 0 && (
                <span className="font-bold">{t("events.todayCount", { count: todayCount })}</span>
              )}
              {todayCount > 0 && soonCount > 0 && " · "}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className={cn("self-center text-xs text-stone-400", !isAdmin && "ml-auto")}>
      ...
      {visible.length === 0 ? (
        <EmptyState
          icon={<CalendarDays className="size-10 opacity-40" />}
        ...
        <div className="space-y-2.5">
          {visible.map((event, i) => (
            <EventCard
              ...
              event={event}
              index={i}
              onCustomEventClick={isAdmin ? handleCustomEventClick : undefined}
            />
          ))}
        </div>
      )}
