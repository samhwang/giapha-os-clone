...
describe("LineageManager", () => {
  ...
  it("calculate shows results table with change summary", async () => {
    ...
    expect(
      screen.getByText(new RegExp(t("lineage.changesSummary", { changed: "\\d+", total: "\\d+" }))),
    ).toBeInTheDocument();
  });
  ...
  it("apply button calls updateBatch with changed records", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          new RegExp(t("lineage.applyChanges", { count: "\\d+" }).replace(/[()]/g, "\\$&"), "i"),
        ),
      ).toBeInTheDocument();
    });
      ...
      expect(mockUpdateBatch).toHaveBeenCalledWith({
        data: {
          updates: expect.arrayContaining([
            expect.objectContaining({ id: "p1", generation: 1 }),
            expect.objectContaining({ id: "p2", generation: 2 }),
          ]),
        },
      });
    });
  ...
  it("shows success message after apply", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(t("lineage.applySuccess", { count: "\\d+" }), "i")),
      ).toBeInTheDocument();
    });
  });
