...
describe('BulkAddChildrenForm', () => {
  ...
  it('renders with one default child row', () => {
    ...
    const nameInputs = screen.getAllByPlaceholderText(new RegExp(t('relationship.fullNamePlaceholder'), 'i'));
  ...
  it('remove row button removes a child row but keeps at least 1', async () => {
    ...
    await user.click(screen.getByText(new RegExp(escRe(t('relationship.addRow')), 'i')));
    expect(screen.getAllByPlaceholderText(new RegExp(t('relationship.fullNamePlaceholder'), 'i'))).toHaveLength(2);

    // Remove one row
    const removeButtons = screen.getAllByRole('button', { name: /✕/ });
    await user.click(removeButtons[0]);
    expect(screen.getAllByPlaceholderText(new RegExp(t('relationship.fullNamePlaceholder'), 'i'))).toHaveLength(1);
  ...
  it('fill child name and submit calls onSubmit with children data', async () => {
    ...
    const nameInput = screen.getByPlaceholderText(new RegExp(t('relationship.fullNamePlaceholder'), 'i'));
    await user.type(nameInput, 'Nguyễn Văn C');

    await user.click(screen.getByRole('button', { name: new RegExp(t('relationship.saveAll'), 'i') }));
