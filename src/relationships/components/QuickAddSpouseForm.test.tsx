...
describe('QuickAddSpouseForm', () => {
  ...
  it('renders name, birth year, and note fields', () => {
    ...
    expect(screen.getByLabelText(new RegExp(escRe(t('relationship.fullNameRequired')), 'i'))).toBeInTheDocument();
    expect(screen.getByLabelText(new RegExp(escRe(t('relationship.birthYearOptional')), 'i'))).toBeInTheDocument();
    expect(screen.getByLabelText(new RegExp(escRe(t('relationship.spouseNoteLabel')), 'i'))).toBeInTheDocument();
  });
  ...
  it('submit calls onSubmit with field values', async () => {
    ...
    await user.type(screen.getByLabelText(new RegExp(escRe(t('relationship.fullNameRequired')), 'i')), 'Trần Thị B');
    await user.type(screen.getByLabelText(new RegExp(escRe(t('relationship.birthYearOptional')), 'i')), '1990');
    await user.type(screen.getByLabelText(new RegExp(escRe(t('relationship.spouseNoteLabel')), 'i')), 'Vợ cả');
