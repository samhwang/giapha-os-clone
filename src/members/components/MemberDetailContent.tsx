          ...
          <div>
            ...
            {person.otherNames && (
              <p className="mt-1.5 text-sm font-medium text-stone-600 italic sm:text-base">
                {t('member.otherNamesLabel')} <span className="font-semibold text-stone-700 not-italic">{person.otherNames}</span>
              </p>
            )}
                ...
                <div className="space-y-1.5 border-l-2 border-stone-100 pl-4">
                  ...
                  {(person.birthYear || person.birthMonth || person.birthDay) && (
                    <p className="flex items-center gap-1.5 text-sm font-medium text-stone-500">
                      <span className="rounded border border-border-default bg-stone-50/80 px-1.5 py-0.5 text-2xs">{t('member.lunarCalendar')}</span>
                      {getLunarDateString({
                        year: person.birthYear,
                        month: person.birthMonth,
                        day: person.birthDay,
                  ...
                  {person.birthMonth && person.birthDay && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      ...
                      {(() => {
                        ...
                        if (!sign) return null;
                        return (
                          <span className="rounded border border-indigo-200/60 bg-indigo-50/80 px-1.5 py-0.5 text-2xs font-bold text-indigo-700">{sign}</span>
                        );
                      })()}
                    </div>
                  )}
                  ...
                  >
                    ...
                    <div className="relative z-10 pl-4">
                      <p className="bg-linear-to-br from-amber-700 to-amber-900 bg-clip-text text-3xl font-black tracking-tight text-transparent sm:text-4xl">
                        {ageData.age}
                        <span className="ml-1.5 text-xs font-bold tracking-wider text-amber-700/60 uppercase sm:text-sm">{t('member.ageSuffix')}</span>
                      </p>
                    </div>
                  </div>
                );
              ...
              {isAdmin ? (
                <div className="rounded-2xl border border-border-strong bg-stone-50 p-5 shadow-sm sm:p-6">
                  <h3 className="mb-4 flex items-center gap-2 border-b border-border-default pb-3 text-sm font-bold text-stone-900 sm:text-base">
                    <span className="rounded-lg border border-amber-200/50 bg-amber-100/80 p-1.5 text-amber-700">🔒</span>
                  ...
                  <dl className="space-y-4 text-sm sm:text-base">
                    <div>
                      ...
                      <dd className="rounded-lg border border-border-default bg-white px-3 py-2 font-medium text-stone-900 shadow-xs">
                        {privateData?.phoneNumber || <span className="font-normal text-stone-400">{t('member.notUpdated')}</span>}
                      </dd>
                    </div>
                    <div>
                      <dt className="mb-1 flex items-center gap-1.5 text-xs-plus font-bold tracking-wider text-stone-500 uppercase">
                      ...
                      <dd className="rounded-lg border border-border-default bg-white px-3 py-2 font-medium text-stone-900 shadow-xs">
                        {privateData?.occupation || <span className="font-normal text-stone-400">{t('member.notUpdated')}</span>}
                      </dd>
                    </div>
                    <div>
                      <dt className="mb-1 flex items-center gap-1.5 text-xs-plus font-bold tracking-wider text-stone-500 uppercase">
                      ...
                      <dd className="rounded-lg border border-border-default bg-white px-3 py-2 font-medium text-stone-900 shadow-xs">
                        {privateData?.currentResidence || <span className="font-normal text-stone-400">{t('member.notUpdated')}</span>}
                      </dd>
                    </div>
                  </dl>
                </div>
          ...
          <div className="group flex flex-col justify-between rounded-xl border border-stone-100 bg-stone-50/80 p-3 transition-colors hover:bg-stone-100/80">
            <div className="mb-2 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-2xs leading-tight font-bold tracking-widest text-stone-500 uppercase">{t('member.biologicalChildren')}</p>
                <p className="mt-0.5 text-xl leading-none font-black text-stone-700">{stats.biologicalChildren}</p>
              </div>
            </div>
        ...
        {(stats.sonInLaw > 0 || stats.daughterInLaw > 0) && (
          <div className="group flex flex-col rounded-xl border border-stone-100 bg-stone-50/80 p-3 transition-colors hover:bg-stone-100/80">
            <div className="mb-2 flex items-center gap-3">
              ...
              <p className="text-2xs font-bold tracking-widest text-stone-500 uppercase">{t('member.inLawLabel')}</p>
            </div>
        ...
        {(stats.paternalGrandchildren > 0 || stats.maternalGrandchildren > 0) && (
          <div className="group flex flex-col rounded-xl border border-stone-100 bg-stone-50/80 p-3 transition-colors hover:bg-stone-100/80">
            <div className="mb-2 flex items-center gap-3">
              ...
              <p className="text-2xs font-bold tracking-widest text-stone-500 uppercase">{t('member.grandchildren')}</p>
            </div>
            <div className="mt-auto w-full space-y-1 border-t border-stone-200/50 pt-1">
              {stats.paternalGrandchildren > 0 && (
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium text-stone-500">{t('member.paternalGrandchildren')}</span>
                  <span className="font-bold text-stone-700">{stats.paternalGrandchildren}</span>
                </div>
              )}
              {stats.maternalGrandchildren > 0 && (
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium text-stone-500">{t('member.maternalGrandchildren')}</span>
