...
vi.mock("./MemberDetailContent", () => ({
  default: ({ person }: { person: { fullName: string } }) => (
    <div data-testid="member-detail-content">{person.fullName}</div>
  ),
}));
...
describe("MemberDetailModal", () => {
  ...
  it("shows error state with close button that resets modal", async () => {
    ...
    const errorCloseButton = closeButtons.find(
      (el) => el.tagName === "BUTTON" && el.closest(".flex-col"),
    );
