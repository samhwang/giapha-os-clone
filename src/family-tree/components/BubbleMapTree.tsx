...
export default function BubbleMapTree({ personsMap, relationships, roots }: BubbleMapTreeProps) {
  ...
  const { nodes, links } = useMemo(() => buildGraphData(roots, personsMap, adj), [roots, personsMap, adj]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      defs.append('clipPath').attr('id', 'avatar-clip').append('circle').attr('r', 26).attr('cx', 0).attr('cy', 0);
      defs.append('clipPath').attr('id', 'avatar-clip-root').append('circle').attr('r', 36).attr('cx', 0).attr('cy', 0);
      ...
      // Draw links
      const link = g.append('g').attr('stroke-opacity', 0.6).selectAll('line').data(links).join('line').attr('stroke', '#d6d3d1').attr('stroke-width', 2);
        ...
        node.each(function (d) {
          ...
          d.people.forEach((person, index) => {
            ...
            avatarGroup
              ...
              .attr(
                'href',
                person.avatarUrl || (person.gender === Gender.enum.male ? `/avatar/${AVATAR_VERSION}/male.svg` : `/avatar/${AVATAR_VERSION}/female.svg`)
              );
          });
        });
      }
