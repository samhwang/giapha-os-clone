...
function StatCard({ label, value, total, icon, color, delay = 0 }: StatCardProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "absolute -top-6 -right-6 size-24 rounded-full opacity-20 blur-2xl transition-opacity group-hover:opacity-30",
          color,
        )}
      />
...
export default function FamilyStats({ persons, relationships }: FamilyStatsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const daughtersInLaw = persons.filter(
      (p) => p.isInLaw && p.gender === Gender.enum.female,
    ).length;
    ...
    for (const p of persons) {
      ...
      if (chineseZodiac)
        chineseZodiacMap.set(chineseZodiac, (chineseZodiacMap.get(chineseZodiac) ?? 0) + 1);
    }
