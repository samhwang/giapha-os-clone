...
describe("FamilyStats", () => {
  it("renders total member count", () => {
    const persons = [
      createPerson({ gender: Gender.enum.male }),
      createPerson({ gender: Gender.enum.female }),
    ];
  ...
  it("renders generation distribution when persons have generation", () => {
    ...
    render(<FamilyStats persons={persons} relationships={[]} />);
    expect(
      screen.getByText(new RegExp(t("stats.generationDistribution"), "i")),
    ).toBeInTheDocument();
  });
  ...
  it("computes married/unmarried from relationships", () => {
    ...
    const marriedCard = screen
      .getByText(new RegExp(t("stats.married"), "i"))
      .closest("div") as HTMLElement;
    expect(within(marriedCard).getByText("2")).toBeInTheDocument();

    const unmarriedCard = screen
      .getByText(new RegExp(t("stats.unmarried"), "i"))
      .closest("div") as HTMLElement;
  ...
  it("renders generation breakdown rows", () => {
    ...
    expect(
      screen.getByText(new RegExp(t("stats.generationLabel", { gen: 1 }), "i")),
    ).toBeInTheDocument();
    expect(
      screen.getByText(new RegExp(t("stats.generationLabel", { gen: 2 }), "i")),
    ).toBeInTheDocument();
    expect(
      screen.getByText(new RegExp(t("stats.generationLabel", { gen: 3 }), "i")),
    ).toBeInTheDocument();
  });

  it("hides generation section when no generations", () => {
    const persons = [
      createPerson({ gender: Gender.enum.male, generation: null }),
      createPerson({ gender: Gender.enum.female, generation: null }),
    ];
    render(<FamilyStats persons={persons} relationships={[]} />);

    expect(
      screen.queryByText(new RegExp(t("stats.generationDistribution"), "i")),
