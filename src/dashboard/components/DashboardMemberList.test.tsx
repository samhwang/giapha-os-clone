...
describe("DashboardMemberList", () => {
  ...
  it("renders search input", () => {
    render(<DashboardMemberList initialPersons={[]} />);
    expect(
      screen.getByPlaceholderText(new RegExp(t("member.searchPlaceholder"), "i")),
    ).toBeInTheDocument();
  });
  ...
  it("renders add member button", () => {
    render(<DashboardMemberList initialPersons={[]} />);
    expect(
      screen.getByRole("button", { name: new RegExp(t("member.addMember"), "i") }),
    ).toBeInTheDocument();
  });
  ...
  it("filters by deceased", async () => {
    const user = userEvent.setup();
    const persons = [
      createPerson({ fullName: "Nguyễn Văn A", isDeceased: true }),
      createPerson({ fullName: "Trần Thị B", isDeceased: false }),
    ];
    render(<DashboardMemberList initialPersons={persons} />);
  ...
  it("filters by firstborn", async () => {
    const user = userEvent.setup();
    const persons = [
      createPerson({ fullName: "Nguyễn Văn A", birthOrder: 1 }),
      createPerson({ fullName: "Trần Thị B", birthOrder: 2 }),
    ];
    render(<DashboardMemberList initialPersons={persons} />);
  ...
  it("sorts by birth year ascending", async () => {
    const user = userEvent.setup();
    const persons = [
      createPerson({ fullName: "Younger", birthYear: 2000 }),
      createPerson({ fullName: "Older", birthYear: 1950 }),
    ];
    render(<DashboardMemberList initialPersons={persons} />);
  ...
  it("sorts by name A-Z", async () => {
    const user = userEvent.setup();
    const persons = [
      createPerson({ fullName: "Trần Văn B" }),
      createPerson({ fullName: "An Văn A" }),
    ];
    render(<DashboardMemberList initialPersons={persons} />);
  ...
  it("renders GenerationGroupedList for generation_desc sort", async () => {
    const user = userEvent.setup();
    const persons = [
      createPerson({ fullName: "Gen1 Person", generation: 1 }),
      createPerson({ fullName: "Gen2 Person", generation: 2 }),
    ];
